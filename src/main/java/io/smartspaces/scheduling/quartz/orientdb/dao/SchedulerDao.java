...
public class SchedulerDao {
  ...
  public void createIndex() {
    schedulerCollection.createIndex(Projections.include(SCHEDULER_NAME_FIELD, INSTANCE_ID_FIELD),
  ...
  public void checkIn() {
    ...
    // If not found Mongo will create a new entry with content from filter and
    // update.
    Document update = createUpdateClause(lastCheckinTime);

    UpdateResult result =
        schedulerCollection.updateOne(schedulerFilter, update, new UpdateOptions().upsert(true));
  ...
  public Scheduler findInstance(String instanceId) {
    ...
    Document doc =
        schedulerCollection.find(createSchedulerFilter(schedulerName, instanceId)).first();
  ...
  public List<Scheduler> getAllByCheckinTime() {
    ...
    schedulerCollection.find().sort(ascending(LAST_CHECKIN_TIME_FIELD))
  ...
  private Document createUpdateClause(long lastCheckinTime) {
    return new Document("$set", new Document().append(LAST_CHECKIN_TIME_FIELD, lastCheckinTime)
  ...
  private Scheduler toScheduler(Document document) {
    return new Scheduler(document.getString(SCHEDULER_NAME_FIELD),
        document.getString(INSTANCE_ID_FIELD), document.getLong(LAST_CHECKIN_TIME_FIELD),
