...
public class ExpiryCalculator {
  ...
  public ExpiryCalculator(StandardSchedulerDao schedulerDao, Clock clock, long jobTimeoutMillis,
  ...
  private boolean isLockExpired(ODocument lockDoc, long timeoutMillis) {
    Date lockTime = lockDoc.field(Constants.LOCK_TIME);
    long elapsedTime = clock.millis() - lockTime.getTime();
    log.debug("Lock time is {}, Elapsed time is {}, timeout is {}",  lockTime, elapsedTime, timeoutMillis);
